...
public class ScreenAPI implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return (上级界面) -> {
            try {
                ConfigBuilder 构建器 = ConfigBuilder.create().setParentScreen(上级界面).setTitle(Text.translatable("identifier-translation.configure.title")).setDoesConfirmSave(false).setSavingRunnable(Manager::保存).setDefaultBackgroundTexture(new Identifier("minecraft", "textures/block/white_concrete.png"));
                ...
                标签样式.add(构建器.entryBuilder().startEnumSelector(Text.translatable("identifier-translation.configure.tag_style.color"), ColorCode.class, Manager.配置项.标签样式.颜色).setDefaultValue(ColorCode.金色).setEnumNameProvider(ColorCode::颜色代码翻译).setSaveConsumer((新值) -> {
                ...
                标签样式.add(构建器.entryBuilder().startBooleanToggle(Text.translatable("identifier-translation.configure.tag_style.hover"), Manager.配置项.标签样式.显示悬停文本).setDefaultValue(true).setSaveConsumer((新值) -> {
                ...
                标识符样式.add(构建器.entryBuilder().startEnumSelector(Text.translatable("identifier-translation.configure.identifier_style.color"), ColorCode.class, Manager.配置项.标识符样式.颜色).setDefaultValue(ColorCode.天蓝色).setEnumNameProvider(ColorCode::颜色代码翻译).setSaveConsumer((新值) -> {
                ...
                标识符样式.add(构建器.entryBuilder().startBooleanToggle(Text.translatable("identifier-translation.configure.identifier_style.hover"), Manager.配置项.标识符样式.显示悬停文本).setDefaultValue(true).setSaveConsumer((新值) -> {
