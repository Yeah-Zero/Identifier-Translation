...
public class ScreenAPI implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return (上级界面) -> {
            try {
                ConfigBuilder 构建器 = ConfigBuilder.create().setParentScreen(上级界面).setTitle(Text.translatable("identifier_translation.configure.title")).setDoesConfirmSave(false).setSavingRunnable(Configuration::保存).setDefaultBackgroundTexture(new Identifier("minecraft", "textures/block/white_concrete.png"));
                ...
                标签样式.add(构建器.entryBuilder().startEnumSelector(Text.translatable("identifier_translation.configure.tag_style.color"), ColorCode.class, Configuration.配置项.标签样式.颜色).setDefaultValue(ColorCode.金色).setSaveConsumer((新值) -> {
                ...
                标签样式.add(构建器.entryBuilder().startBooleanToggle(Text.translatable("identifier_translation.configure.tag_style.hover"), Configuration.配置项.标签样式.显示悬停文本).setDefaultValue(true).setSaveConsumer((新值) -> {
                ...
                标识符样式.add(构建器.entryBuilder().startEnumSelector(Text.translatable("identifier_translation.configure.identifier_style.color"), ColorCode.class, Configuration.配置项.标识符样式.颜色).setDefaultValue(ColorCode.天蓝色).setSaveConsumer((新值) -> {
                ...
                标识符样式.add(构建器.entryBuilder().startBooleanToggle(Text.translatable("identifier_translation.configure.identifier_style.hover"), Configuration.配置项.标识符样式.显示悬停文本).setDefaultValue(true).setSaveConsumer((新值) -> {
